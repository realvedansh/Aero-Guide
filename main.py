...
def create_app():
    ...
    def ask_ai():
        try:
            ...
            # Notes-generation prompts (from the Smart Notes creator) are long, near-identical
            # boilerplate across every topic — only the topic name differs. That near-identical
            # text was fooling the adaptive-learning similarity match into treating two
            # different topics (e.g. "Electrostatics" vs "Capacitance") as "the same question
            # already answered", and serving back the OLD topic's cached answer. The frontend
            # now sends mode:"notes" for these requests so we can skip the KB lookup/learn
            # step entirely and always ask the model fresh — normal chat (mode:"chat", the
            # default) keeps using the KB as before.
            ...
            weather_keywords = ("weather", "mausam", "temperature", "taapman", "rain", "barish", "garmi", "sardi","news")
            ...
            #    Skipped for notes-generation calls — their boilerplate prompt text is what
            #    caused the cross-topic mismatch in the first place, so we don't want it
            #    getting saved into the KB either.
            if source == "model" and mode != "notes":
    ...
    # ------------------------------------------------------------------
    # Text-to-speech — ElevenLabs (Hindi via eleven_multilingual_v2)
    # ------------------------------------------------------------------
    # The API key lives only here on the server (read from the ELEVENLABS_API_KEY env
    # var) — it must never be shipped in frontend JS, since anything in the browser is
    # visible to every visitor and the key could be copied and used by anyone. If the key
    # isn't configured, or the ElevenLabs call fails for any reason (quota, network, bad
    # key), this returns a clear error and the frontend falls back to the browser's own
    # free speechSynthesis voice instead of breaking.
    ELEVENLABS_DEFAULT_VOICE_ID = "21m00Tcm4TlvDq8ikWAM"  # "Rachel" — supports Hindi via the multilingual model; overridden by ELEVENLABS_VOICE_ID env var if set
    ...
    def text_to_speech():
        ...
        if not api_key:
            return jsonify({"error": "ElevenLabs TTS not configured on server"}), 503

        data = request.get_json(silent=True) or {}
        text = sanitize_input(data.get("text", ""))[:1000]
        if not text:
            return jsonify({"error": "No text provided"}), 400
        voice_id = data.get("voice_id") or os.getenv("ELEVENLABS_VOICE_ID") or ELEVENLABS_DEFAULT_VOICE_ID
